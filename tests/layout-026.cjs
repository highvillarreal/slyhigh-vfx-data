...
(async()=>{
 ...
 for(const [width,height,top,right,bottom,left] of [[320,568,0,0,0,0],[375,667,20,0,0,0],[393,852,59,0,34,0],[402,874,62,0,34,0],[430,932,62,0,34,0],[667,375,0,0,0,0],[874,402,0,59,21,59],[744,1133,24,0,20,0],[768,1024,24,0,20,0],[820,1180,24,0,20,0],[1024,768,24,0,20,0],[1366,1024,0,0,0,0]]){
  ...
  for(const language of ['es','en']){
   ...
   for(const target of ['shoot','shot','library','tools','export','screen','distortion','projectSettings']){
    ...
    assert.ok(Math.abs(m.body.height-height)<1,label);
    ...
    const lower=target==='shot'?m.dock.top:m.hidden?height:m.nav.top;
    assert.ok(Math.abs(m.main.bottom-lower)<1,label);
    ...
    if(target==='shoot'){
     ...
     assert.ok(await page.evaluate(()=>document.getElementById('main').scrollTop>0),label);
     assert.equal(await page.evaluate(()=>scrollY),0,label);
    }
    checks++;
   }
  }
 ...
 const listScroll=await page.evaluate(()=>document.getElementById('main').scrollTop);
 await twentieth.click();await page.waitForSelector('#sh');
 await page.evaluate(()=>back());await page.waitForFunction(()=>route.page==='shoot');
 assert.ok(await page.evaluate(y=>Math.abs(document.getElementById('main').scrollTop-y)<2,listScroll));
 ...
 assert.ok(await page.evaluate(()=>Math.abs(document.body.getBoundingClientRect().height-430)<1));
 ...
 await page.screenshot({path:'test-output/layout-026-'+(engine===webkit?'webkit':'chromium')+'-list.png'});
 await page.evaluate(()=>go('shot'));await page.screenshot({path:'test-output/layout-026-'+(engine===webkit?'webkit':'chromium')+'-shot.png'});
