...
(async()=>{
 ...
 const report=JSON.parse(await page.evaluate(()=>copiedDiagnostics));assert.equal(report.build,'0.2.9');assert.ok(report.initial&&report.current);assert.ok(report.recent.length<=15);assert.doesNotMatch(JSON.stringify(report),/IPSY|camera-qa|lens-qa|PRODUCCION/);
